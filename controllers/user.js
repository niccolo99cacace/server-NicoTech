...
exports.signIn = async (req, res, next) => {
  try {
    ...
    if (user.confirmedAccount == false)
      return sendError(res, "Email still not confirmed");
  ...
  try {
    ...
    User.findOne({ email: email }, (err, user) => {
      ...
      if (err || !user) {
        ...
        return res
          .status(400)
          .json({ message: "There is no user matching this email." });
      } else {
        ...
        const token = jwt.sign(
          { _id: userId },
          process.env.JTW_TOKEN_SIGNATURE,
          { expiresIn: "15m" }
        );
        ...
        User.findOneAndUpdate(
          ...
          (err, user) => {
            if (err) {
              console.log(
                "Errore nella memorizzazione del resetPasswordToken nel database"
              );
              return res
                .status(400)
                .json({
                  message: "Error storing resetPasswordToken in database",
                });
            } else {
              ...
              transporter.sendMail(mailOptions, (err, info) => {
                if (err) {
                  ...
                  res
                    .status(200)
                    .json({
                      ok: "A password reset link has been sent to your email.",
                    });
                }
              });
            }
  ...
  try {
    ...
    jwt.verify(token, process.env.JTW_TOKEN_SIGNATURE, (err, decodedToken) => {
      if (err) {
        ...
        return res
          .status(400)
          .json({ message: "TOKEN RESET PASSWORD NON VALIDO" });
      } else {
        ...
        User.findById(decodedToken._id, (err, user) => {
          if (err || !user) {
            ...
            return res
              .status(400)
              .json({ message: "UTENTE NON TROVATO CON IL RESET TOKEN USATO" });
          } else {
  ...
  try {
    ...
    jwt.verify(token, process.env.JTW_TOKEN_SIGNATURE, (err, decodedToken) => {
      if (err) {
        ...
        return res
          .status(400)
          .json({ message: "RESET PASSWORD TOKEN USATO NON VALIDO" });
      } else {
        // Cerca l'utente nel database utilizzando l'ID contenuto nel token
        User.findById(decodedToken._id, (err, user) => {
          if (err || !user) {
            // Se l'utente non è stato trovato, reindirizza l'utente a una pagina di errore
            console.log(
              "L'UTENTE CERCATO IN BASE AL RESET PASSWORD TOKEN NON è STATO TROVATO"
            );
            return res
              .status(400)
              .json({
            ...
            if (newPassword.newPassword === "") {
              ...
              return res
                .status(400)
                .json({ message: "IL CAMPO NEWPASSWORD é VUOTO" });
            } else {
              ...
              user.save((err, updatedUser) => {
                if (err) {
                  // Gestisci l'errore
                  console.log(
                    "ERRORE DURANTE IL CARICAMENTO DELLA NUOVA PASSWORD NEL DATABASE"
                  );
                  console.log(err);
                  return res
                    .status(400)
                  ...
                  return res
                    .status(200)
                    .json({ ok: "PASSWORD SUCCESSFULLY CHANGED" });
                }
              });
            }
          }
